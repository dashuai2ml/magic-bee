...
void GIFDraw(GIFDRAW *pDraw)
{
  ...
  if (pDraw->ucHasTransparency) { // if transparency used
    ...
    while(x < iWidth) {
      ...
      if (iCount) { // any opaque pixels?
          for(int xOffset = 0; xOffset < iCount; xOffset++ ){
            dma_display->drawPixel(PANEL_RES_X-x-xOffset, PANEL_RES_Y-y, usTemp[xOffset]); // 565 Color Format
          }
        x += iCount;
        iCount = 0;
      }
    ...
    for (x=0; x<iWidth; x++)
      dma_display->drawPixel(PANEL_RES_X-x, PANEL_RES_Y-y, usPalette[*s++]); // color 565
      /*
